...
Assume that risk is balanced between the $m$ and 1 fragment cases. As risk is biodiversity times the
perimeter, then assuming the perimeter is proportional to the square root of area, we see that
$$
...
If $z < \frac{1}{2}$ then $z - \frac{1}{2} < 0$, and given $m$ fragments with $m \geq 1$ we have
$$
m^{z - \frac{1}{2}} < 1
$$
It is clear that
...
Further, in the case where $z=\frac{1}{2}$ we have
$$
BR(m) = 1 - \left(1 - \frac{1}{\sqrt{m}}\right)^m
$$
and
$$
RR(m) = \left[1 - \left(1 - \frac{1}{\sqrt{m}}\right)^m\right]^{-2}
$$
so that it is clear that $BR(m) \rightarrow 1^{-}$ and $RR(m) \rightarrow 1^{+}$ as $m \rightarrow \infty$.

## Extension to the case where we have dilution or amplification
