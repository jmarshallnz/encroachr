...
$$
\mathsf{Risk}(1, A) = A^{az} k A^{1/2} = k A ^ {az + 1/2}
$$
for some constant $k$, where we have included a dilution/amplification effect $a$. In the case where we have $m$ fragments, the total risk is the sum of the individual risks, and hence
$$
\mathsf{Risk}(m, \hat{A}) = m \mathsf{Risk}(1, \frac{\hat{A}}{m}) = m k \left(\frac{\hat{A}}{m}\right)^{az + 1/2}
$$
...
m k \left(\frac{\hat{A}}{m}\right)^{az + 1/2} &= k A ^ {az + 1/2}\\
\left(\frac{\hat{A}}{m}\right)^{az + 1/2} &= \frac{A ^ {az + 1/2}}{m}\\
\frac{\hat{A}}{m} &= \frac{A}{m^\frac{1}{az+1/2}}\\
\hat{A} &= A\frac{m}{m^\frac{1}{az+1/2}}\\
        &= A m^\frac{az-1/2}{az+1/2}
\end{aligned}
$$
...
&= m^{z\frac{az-1/2}{az+1/2}}\left[1 - \left( 1 - \frac{1}{m^z} \right)^m\right].
\end{aligned}
$$
...
$$
\begin{aligned}
\mathsf{RR}(m) &= \frac{m k \left(\frac{\tilde{A}}{m}\right)^{az + 1/2}}{k A ^ {az + 1/2}}\\
& = \frac{m k \left(\frac{A}{m\left[1 - \left( 1 - \frac{1}{m^z} \right)^m\right]^\frac{1}{z}}\right)^{az + 1/2}}{k A ^ {az + 1/2}}\\
& = \frac{m}{m^{az + 1/2}\left[1 - \left( 1 - \frac{1}{m^z} \right)^m\right]^\frac{az + 1/2}{z}}\\
& = \frac{1}{m^{az - 1/2}\left[1 - \left( 1 - \frac{1}{m^z} \right)^m\right]^\frac{az + 1/2}{z}}.\\
\end{aligned}
$$
...
If $z < \frac{1}{2a}$ then $az - \frac{1}{2} < 0$, and given $m$ fragments with $m \geq 1$ we have
$$
m^{az - \frac{1}{2}} < 1
$$
It is clear that
...
$$
with equality holding only if $z = 0$. From this, and noting that the fraction $\frac{z}{z+1/2}$ is non-negative it is clear that $RR(m) > 1$ and $BR(m) < 1$ for $0 < z < \frac{1}{2a}$. As an amplification effect does not seem plausible, $a \leq 1$ and thus for the range of powers of $z$ observed ($z < \frac{1}{2}$) we see that either risk increases or biodiversity decreases with fragmentation.

Further, in the case where $z=\frac{1}{2a}$ we have
$$
BR(m) = 1 - \left(1 - \frac{1}{m^{1/{2a}}}\right)^m
$$
and
$$
RR(m) = \left[1 - \left(1 - \frac{1}{m^{1/{2a}}}\right)^m\right]^{-2}
$$
so that it is clear that $BR(m) \rightarrow 1^{-}$ and $RR(m) \rightarrow 1^{+}$ as $m \rightarrow \infty$. Thus, only larger values of $z$ that have not been observed in practice allow risk and biodiversity to remain balanced as fragmentation increases.
